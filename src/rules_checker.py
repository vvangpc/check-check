...
class RulesChecker:
    ...
    def _check_reference_numerals(self, claims_text, specs_text):
        ...
        # ===== 核心函数：从正则捕获的原始中文文本中提取真实部件名 =====
        # 策略：从右往左扫描"语义断点"，找到最右侧的动词/介词/连词，取其后的文本
        # 关键改进：双层断点——"安全断点"始终生效，"激进断点"仅当原始文本较长时才生效
        # 这样可以避免 "控制系统" 被过度剥离为 "系统"
        def clean_name(raw_name):
            """
            从贪婪匹配的原始中文字符串中提取真实的部件名称。
            例: "一方面便于分别吊装浮体" → "浮体"
                "密封连接后将调节后的气体导入阀体" → "阀体"
                "控制系统" → "控制系统" （不会过度剥离）
            """
            ...
            # ==== 第一层：安全断点 ====
            # 这些词在任何情况下都不可能是部件名的组成部分，可以无条件切割
            safe_breaks = [
                # 指示词 / 限定词
                '所述', '该',
                # 介词/连词（绝对安全的单字断点）
                ...
                '根据', '基于', '对于',
                # 时序/逻辑词
                '然后将', '然后', '首先', '其次', '接着', '随着', '从而',
                '确保后续',
                # "可/不/无需/能够+动词"组合
                '可在', '可避免', '可通过', '不在', '而在', '需在',
                '无需在', '能够',
                # === 纯动词/动词短语（绝不可能出现在部件名中） ===
                '通过', '经过', '穿过', '越过',         # 经由动词
                '避免', '导致', '确保', '利用', '针对',  # 操作/判断动词
                '拆除', '吊装', '采用', '具有',          # 工程操作词
                '克服', '脱离', '贯穿',                  # 力学/空间动词
                '推出', '顶出', '滑入', '伸入', '装入',  # 运动动作
                '嵌入', '旋入', '进入',                  # 进出动作
            ...
            # ==== 第二层：激进断点 ====
            # 这些动词经常是部件名的组成部分（如"控制系统"、"密封圈"、"连接件"）
            # 仅当原始文本较长（>6字符，意味着有上下文语境混入）时才启用
            aggressive_breaks = [
                '控制', '检测', '检查',   # 控制系统/检测机构
                '密封', '连接', '导入',   # 密封圈/连接件/导入管
                '安装', '设置', '固定',   # 安装架/设置模块
                '调节', '贴合', '导向',   # 调节阀/贴合面/导向片
            ]

            # 决定使用哪些断点
            active_breaks = list(safe_breaks)
            if len(raw_name) > 6:
                active_breaks.extend(aggressive_breaks)

            # 从右往左扫描，找最右侧的断点
            best_cut = 0
            for bp in active_breaks:
                pos = raw_name.rfind(bp)
                if pos != -1:
                    cut = pos + len(bp)
                    if cut > best_cut and len(raw_name) - cut >= 2:
                        best_cut = cut

            name = raw_name[best_cut:] if best_cut > 0 else raw_name

            # Phase 2: 如果经过切割，残留了前导介词（在/于/至/到），继续剥离
            if best_cut > 0:
                for prep in ['在', '于', '至', '到']:
                    if name.startswith(prep) and len(name) > len(prep) + 1:
                        name = name[len(prep):]

            # Phase 3: 剥离前导指示词（所述、该），phase 1 可能没切到的情况
            for det in ['所述', '该', '此', '本']:
                if name.startswith(det) and len(name) > len(det) + 1:
                    name = name[len(det):]
